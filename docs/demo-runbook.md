...
## 7. 로컬 온톨로지 / Foundry-ready 관점(2026-07-04 착수, 진행 중)

이번 세션에서 `src/ontology/`에 D4D 데이터를 Palantir Foundry 온톨로지와
유사한 형태(Object/Link/Action)로 모델링하는 **로컬 전용** 계층 작업을
시작했다. 아래는 현재 상태를 있는 그대로 기록한 것이며, **Foundry가
이미 연동되어 있다는 뜻이 아니다** — Foundry 계정/Developer Console
enrollment는 아직 존재하지 않고, `@osdk/*` 등 Palantir SDK도 설치되지
않았으며, 네트워크 호출이나 인증 흐름도 없다.

**확인됨(구현 완료)**:
- `src/ontology/contracts.ts` — Sensor/Observation/Track/Incident 등
  객체·링크·액션 이름에 대한 Zod 계약과 readonly 타입 정의.
- `docs/ontology/palantir-foundry-mapping.md`,
  `docs/ontology/palantir-foundry-manifest.json` — 로컬 객체/링크/액션과
  향후 Foundry 객체/링크/액션 타입 후보 이름을 1:1로 매핑한 문서·매니페스트.
  현재 시점에 "구현됨"과 "Foundry enrollment 이후로 이연됨"을 구분해 표기.

**미완료(다음 세션 필요)**:
- 실제 D4D 런타임 데이터(카메라 토폴로지, 관측, 트랙, 사건, 증거, 대응
  게이트)를 온톨로지 객체/링크로 변환하는 어댑터(`localObjects.ts`,
  `localLinks.ts`)와 로컬 액션 구현(`actions.ts`), Foundry 어댑터 seam
  (`foundryAdapter.ts`)은 아직 코드가 없고, 이를 참조하는 테스트만
  존재해 `npm run test` 실행 시 모듈을 찾지 못해 실패한다(§8 참고).
  이 상태에서는 기존 COP 대시보드 동작(카메라 월, 활동 스트림, 우측 레일
  그래프)에는 영향이 없다 — 온톨로지 모듈은 아직 어떤 런타임 코드에서도
  import되지 않는다.
- 위 어댑터들이 완성되기 전까지 이 절은 피칭 각본(§2)에 반영하지 않는다.
